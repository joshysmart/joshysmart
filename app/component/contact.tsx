...
const Contact = (props:{actionData:any}) => {
  ...
  return (
    <div id="contact" className="flex flex-col py-16 pb-20 md:max-w-md md:m-auto lg:flex-row lg:m-0 lg:justify-between lg:min-w-full lg:px-8 xl:px-40">
      ...
      <div className="right-section lg:w-2/5 relative">
        <Form method="post" className="flex flex-col px-4 text-pureWhite " >
          ...
          <span className="text-right block mt-1 text-red-500" aria-live="polite">{error?.name && error?.name}</span>
          <label htmlFor="email" className="relative">
            <input id="email" type="email" name="email"placeholder="EMAIL" className={`p-4 text-base font-medium border-b pt-8 outline-none bg-transparent focus:border-lightGreen w-full ${error?.email && "border-red-500"}`} defaultValue={user?.email} required/>
